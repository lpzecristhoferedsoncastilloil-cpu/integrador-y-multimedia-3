...
export default function Sidebar() {
  ...
  return (
    <div className="w-[260px] h-screen bg-gray-950 text-white fixed left-0 top-0 flex flex-col z-40">
      ...
      <div className="px-4 py-4 border-b border-white/10">
        <div className="flex items-center gap-3 bg-white/5 rounded-2xl p-3">
          <div className="w-10 h-10 bg-gradient-to-br from-indigo-400 to-purple-500 rounded-xl flex items-center justify-center font-bold text-sm">
            {iniciales}
          </div>
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-sm truncate">{usuario?.nombre_usuario}</p>
            <p className="text-xs text-indigo-400 capitalize">{usuario?.rol_usuario}</p>
          </div>
